...
- **Functionality**: Advanced AI-powered pressing identification system that combines multi-pass image analysis with enhanced OCR (including Tesseract.js for precision text extraction), intelligent pattern recognition for matrix/runout codes, manual hints, and **real-time Discogs database API integration** to generate ranked pressing candidates with comprehensive evidence, confidence scoring, and visual verification. NEW: Matrix code pattern recognition using regex patterns for common formats (A1/B1, SHVL-804-A-1, etc.), duplicate detection to prevent redundant candidates, enhanced evidence synthesis combining multiple signals, smart fallback strategies when Discogs API unavailable, and visual pressing verification showing user-uploaded images alongside Discogs database images for manual confirmation
- **Purpose**: Eliminates tedious manual research and dramatically improves cataloging accuracy by automatically identifying specific pressings from uploaded images and metadata clues, enhanced with real Discogs database matching for verified pressing data. Multi-layered intelligence combines OCR precision, pattern matching, AI reasoning, and database lookups to achieve 85%+ accuracy on clear images
- **Trigger**: User clicks "Identify Pressing" button in add item dialog or accesses standalone pressing identification workflow
- **Progression**: Upload images (front/back cover, labels, runouts) → **System automatically performs Tesseract OCR on all images for precise text extraction** → Enter manual OCR runout values if known (pre-populated from Tesseract results) → Add optional manual hints (artist, title, catalog number, country, year, format, label) → **Toggle Discogs database search (enabled by default if API token configured)** → Click "Identify Pressing" → **System queries Discogs database API for matching releases if enabled** → AI analyzes all images for text, catalog numbers, matrix codes with enhanced pattern matching, label names → **Tesseract OCR results enhance matrix/runout extraction accuracy** → Extracts and normalizes identifiers using regex patterns → **Cross-references image data with real Discogs release information** → **Deduplicates candidates by Discogs ID and normalized identifiers** → Scores candidates based on catalog number match (highest priority), barcode match, matrix/runout similarity with fuzzy matching, country match, format match, label similarity, year plausibility → **Synthesizes evidence from multiple sources into clear explanations** → Returns ranked top 3-5 candidates with confidence bands (high 80%+/medium 60-79%/low 40-59%/ambiguous <40%) → Each candidate displays evidence snippets explaining matches, **indicates if sourced from Discogs database**, and shows matched identifiers → **NEW: User expands candidate to view pressing verification comparison with side-by-side images** → User selects best candidate after visual confirmation or chooses "none of these" for ambiguous results → **Fallback to pure AI suggestions when API unavailable or no database matches**
- **Success criteria**: Analysis completes in <20s for 3-6 images + Tesseract OCR + Discogs API calls, correctly identifies pressing in 85%+ of cases with clear images and database matches (up from 80%), provides detailed evidence for each candidate with clear reasoning, confidence bands accurately reflect match quality with calibrated thresholds, supports ambiguous outcomes without fabricating certainty, gracefully handles incomplete data, manual hints improve results when images are unclear, **Discogs database matches appear with high confidence and include Discogs release IDs**, **Tesseract OCR improves matrix/runout extraction accuracy by 30%**, **pattern recognition successfully identifies common matrix formats in 90%+ of cases**, **duplicate detection eliminates redundant candidates**, **visual verification UI helps users confidently select correct pressing**, system falls back to AI-generated suggestions when API not configured or no matches found with clear indicators
